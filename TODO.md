...
- [~] Fix release automation end-to-end. Done 2026-07-10 (release 0.0.139): (1) `publish_helm` was also racing the main-branch `-latest` workflow on the `gh-pages` index push — both chart-releaser jobs now share a `concurrency: helm-chart-release` group and use `skip_existing: true` for idempotent re-runs. (2) The GitHub Actions app was added to `main`'s branch-protection bypass list so the `update_argocd` job's push to `main` succeeds. Still open: prd Application manifests are NOT app-of-apps managed, so after the workflow commits the targetRevision bump someone must still `kubectl apply -f infra/argocd/applications/pickem-prd.yaml` — an app-of-apps Application watching `infra/argocd/applications/` would retire that manual step.
