...
- [ ] Make the "Create Family" flow more involved so people are less likely to create a family just for fun. Require the creator to set **all** pool rules/settings during signup (the full `PoolSettings` form inline, rather than accepting silent defaults to tweak later). Include a placeholder field for entering the emails of users they want to invite — non-functional for now, just captured/ignored until email invites exist (wire it up alongside the email-invite item above). Goal: raise the effort/intent bar for creating a family.
