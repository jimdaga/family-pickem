...
replica — see `scheduler.enabled` in the Helm chart). The superadmin console's
Jobs page (`/superadmin/jobs/`) can also queue a one-off run of any allowlisted
command through the same scheduler; see `pickem_superadmin/jobs.py`.
